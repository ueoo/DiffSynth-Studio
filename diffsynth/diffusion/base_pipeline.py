...
from ..core import AutoTorchModule, AutoWrappedLinear, ModelConfig, load_state_dict
    ...
    def load_lora(
        ...
        if state_dict is None:
            if isinstance(lora_config, str):
                lora_raw = load_state_dict(lora_config, torch_dtype=self.torch_dtype, device=self.device)
            else:
                lora_config.download_if_necessary()
                lora_raw = load_state_dict(lora_config.path, torch_dtype=self.torch_dtype, device=self.device)
        else:
            lora_raw = state_dict
        lora_loader = self.lora_loader(torch_dtype=self.torch_dtype, device=self.device)
        raw_keys = list(lora_raw.keys()) if isinstance(lora_raw, dict) else []
        lora = lora_loader.convert_state_dict(lora_raw)
        if hotload is None:
        ...
        if hotload:
            ...
            for _, module in module.named_modules():
                if isinstance(module, AutoWrappedLinear):
                    ...
                    if len(linear_names_seen) < 50:
                        linear_names_seen.append(name)
            ...
            if updated_num == 0:
                # Diagnostics: show why nothing matched
                lora_layer_names = sorted(
                    {
                        k.rsplit(".lora_", 1)[0]
                        for k in lora.keys()
                ...
                print("LoRA debug: no matches found between checkpoint keys and AutoWrappedLinear.name.")
                print(f"LoRA debug: converted LoRA keys={len(lora)}; inferred LoRA layers={len(lora_layer_names)}.")
                if raw_keys:
                    # Common prefix signals
                    raw_key_str = "\n".join(raw_keys[: min(len(raw_keys), 5000)])
                    for prefix in (
                        "base_model.model.",
                        "module.",
                        "pipe.dit.",
                        ...
                        if prefix in raw_key_str:
                            print(f"LoRA debug: raw checkpoint contains prefix '{prefix}' (likely training wrapper).")
                if lora_layer_names:
                    print(f"LoRA debug: example LoRA layer name: {lora_layer_names[0]}")
                if linear_names_seen:
                    print(f"LoRA debug: example runtime layer name: {linear_names_seen[0]}")
                print(
                    "LoRA debug: if the example names don't share a common suffix/prefix, the checkpoint naming doesn't match this model."
                )
        else:
