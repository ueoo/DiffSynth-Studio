...
def add_video_size_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Randomly sample a contiguous window of num_frames from videos (instead of always taking the prefix).",
    )
    return parser
...
def add_training_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--start_epoch", type=int, default=0, help="Start epoch for training loop (used for resume).")
    parser.add_argument(
        "--start_step", type=int, default=0, help="Start step counter (used for step-based checkpoint naming)."
    )
    parser.add_argument(
        "--auto_resume",
        default=False,
        action="store_true",
        help="Auto-resume from the latest LoRA checkpoint in output_path.",
    )
    parser.add_argument(
