...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # Masked SDEdit (V2V): mask==1 keeps generated (edit), mask==0 keeps source (noised to current timestep)
        sdedit_mask_video: Optional[list[Image.Image]] = None,
        # Speech-to-video
        ...
        inputs_shared = {
            ...
            "sdedit_mask_video": sdedit_mask_video,
        ...
        # Prepare masked SDEdit constraint (only for V2V)
        mask_latents = None
        if sdedit_mask_video is not None and inputs_shared.get("input_video") is not None:
            if not isinstance(sdedit_mask_video, list):
                raise TypeError(f"sdedit_mask_video must be a list of PIL.Image, but got {type(sdedit_mask_video)}")
            if len(sdedit_mask_video) == 0:
                raise ValueError("sdedit_mask_video must be a non-empty list of PIL.Image")
            if "input_latents" not in inputs_shared:
                raise RuntimeError(
                    "Masked SDEdit requires `input_latents` but it was not produced. "
                    "This usually indicates an internal pipeline mismatch."
                )
            if "noise" not in inputs_shared:
                raise RuntimeError(
                    "Masked SDEdit requires `noise` but it was not produced. "
                    "This usually indicates an internal pipeline mismatch."
                )

            # Resize & match frame length, then preprocess to [0,1] tensor.
            mask_frames = [m.resize((width, height)).convert("RGB") for m in sdedit_mask_video]
            if len(mask_frames) >= num_frames:
                mask_frames = mask_frames[:num_frames]
            else:
                mask_frames.extend([mask_frames[-1]] * (num_frames - len(mask_frames)))
            mask_pixel_values = self.preprocess_video(mask_frames, min_value=0, max_value=1).to(
                device=self.device, dtype=self.torch_dtype
            )
            # Use a single channel mask: [B, 1, T, H, W]
            mask_pixel_values = mask_pixel_values[:, 0:1]

            latents = inputs_shared["latents"]
            lat_t, lat_h, lat_w = latents.shape[2], latents.shape[3], latents.shape[4]
            mask_latents = F.interpolate(mask_pixel_values, size=(lat_t, lat_h, lat_w), mode="nearest")
            # Binarize after resize to avoid soft borders from resampling
            mask_latents = (mask_latents > 0.5).to(device=latents.device, dtype=latents.dtype)

            if inputs_shared["input_latents"].shape != latents.shape:
                raise ValueError(
                    f"Masked SDEdit shape mismatch: input_latents={tuple(inputs_shared['input_latents'].shape)} "
                    f"vs latents={tuple(latents.shape)}"
                )
        ...
        for progress_id, timestep in enumerate(self.scheduler.timesteps):
            ...
            # Masked SDEdit: keep background latents locked to the source video (noised to current timestep)
            if mask_latents is not None:
                ...
                inputs_shared["latents"] = inputs_shared["latents"] * mask_latents + bg_latents * (1 - mask_latents)
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def _resize_and_match_length(self, frames, *, num_frames: int, width: int, height: int):
        """
        Normalize video frames for V2V(SDEdit):
        - resize every frame to (width, height)
        - truncate or pad (repeat last frame) to exactly num_frames
        """
        if frames is None:
            return None
        if not isinstance(frames, list):
            # keep this strict: pipeline type hints expect list[PIL.Image]
            raise TypeError(f"input_video must be a list of PIL.Image, but got {type(frames)}")
        if len(frames) == 0:
            return None

        resized = [f.resize((width, height)) for f in frames]
        if len(resized) >= num_frames:
            return resized[:num_frames]
        last = resized[-1]
        resized.extend([last] * (num_frames - len(resized)))
        return resized
    ...
    def process(
        ...
        input_video = self._resize_and_match_length(input_video, num_frames=num_frames, width=width, height=height)
        if input_video is None:
            return {"latents": noise}
        ...
        if vace_reference_image is not None:
            if not isinstance(vace_reference_image, list):
                vace_reference_image = [vace_reference_image]
            vace_reference_image = self._resize_and_match_length(
                vace_reference_image, num_frames=len(vace_reference_image), width=width, height=height
            )
        ...
        if pipe.scheduler.training:
            ...
            # For masked SDEdit we also need the clean input latents during denoising.
            if sdedit_mask_video is not None:
...
class WanVideoUnit_VAP(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, inputs_shared, inputs_posi, inputs_nega):
        if inputs_shared.get("vap_video") is None:
            ...
            vap_prompt, negative_vap_prompt = inputs_posi.get("vap_prompt", ""), inputs_nega.get(
                "negative_vap_prompt", ""
            )
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_motion_latents(
        ...
        if motion_video is not None and len(motion_video) > 0:
            assert (
                len(motion_video) == motion_frames
            ), f"motion video must have {motion_frames} frames, but got {len(motion_video)}"
...
def model_fn_wans2v(
    ...
    for block_id, block in enumerate(dit.blocks):
        if use_gradient_checkpointing_offload:
            ...
            x = dit.after_transformer_block(
                block_id, x, audio_emb_global, merged_audio_emb, seq_len_x_global, use_unified_sequence_parallel
            )
