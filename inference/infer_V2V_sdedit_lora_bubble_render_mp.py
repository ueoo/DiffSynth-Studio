...
def _run(cmd: str) -> int:
    ...
    # Run through shell so env prefix works as written
...
def launch(args):
    ...
    for gpu_idx, gpu_id in enumerate(gpu_ids):
        # Each worker sees exactly one GPU as cuda:0
        env = f"CUDA_VISIBLE_DEVICES={gpu_id}"
        cmd = (
            f"{env} python inference/infer_V2V_sdedit_lora_bubble_render.py"
...
if __name__ == "__main__":
    ...
    # Either specify explicit ids (recommended) or a count.
