#!/usr/bin/env bash
# Finetune Wan 2.2 I2V on Blender flowers (dahlia). Uses dataset renders/ with
# per-iteration random start frame, frame_step=2, random view; frames are
# padded to Wan aspect ratio and resized to 832x480.
# Run from DiffSynth-Studio repo root. Create metadata first if needed:
#   python state_processing/wan/create_blender_flowers_metadata.py \
#     --dataset_base_path "${DATASET_BASE_PATH}" --flower_name dahlia
set -euo pipefail
...
DATASET_BASE_PATH="${DATASET_BASE_PATH:-/scr/yuegao/TRELLIS_datasets/BlenderFlowers_Dahliamore20kfull_sub_merged}"
METADATA_PATH="${METADATA_PATH:-${DATASET_BASE_PATH}/metadata_wan.csv}"
OUTPUT_PATH="${OUTPUT_PATH:-/scr/yuegao/wan_models/train_blenderflowers/dahlia_high}"
