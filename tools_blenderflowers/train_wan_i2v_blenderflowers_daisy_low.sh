#!/usr/bin/env bash
# Finetune Wan 2.2 I2V *low-noise* LoRA on Blender flowers (daisy). Same data as
# train_wan_i2v_blenderflowers_daisy_high.sh but trains the low-noise model branch.
set -euo pipefail
...
DATASET_BASE_PATH="${DATASET_BASE_PATH:-/scr/yuegao/TRELLIS_datasets/BlenderFlowers_Daisymore20kfull_sub_merged}"
METADATA_PATH="${METADATA_PATH:-${DATASET_BASE_PATH}/metadata_wan.csv}"
OUTPUT_PATH="${OUTPUT_PATH:-/scr/yuegao/wan_models/train_blenderflowers/daisy_low}"
